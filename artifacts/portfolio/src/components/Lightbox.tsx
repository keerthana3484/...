...
interface LightboxProps {
  ...
  onNext: () => void;
  onPrev: () => void;
  poster: { title: string; gradient: string; category: string } | null;
}
        ...
        >
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-background/20">
              <h2 className="text-4xl md:text-6xl font-serif text-white font-bold drop-shadow-lg">{poster.title}</h2>
              <p className="mt-4 text-lg uppercase tracking-[0.2em] text-primary/90 font-medium">{poster.category}</p>
            </div>
          </motion.div>
        </motion.div>
      )}
